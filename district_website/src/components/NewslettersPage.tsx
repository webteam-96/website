...
export default function NewslettersPage({ variant }: { variant: 'governor' | 'club' }) {
  ...
  const items = useMemo(() => {
    ...
    if (!q) return all
    return all.filter((n) => [n.title, n.date, n.description].some((f) => f.toLowerCase().includes(q)))
  }, [all, query])
      ...
      <div className="mx-auto max-w-[1280px] px-5 py-8 sm:px-8">
        ...
        <div className="relative mb-6 max-w-md">
          ...
          <input
            ...
            placeholder={variant === 'governor' ? 'Search by month…' : 'Search by club or period…'}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="truncate text-[15px] font-bold text-brand-bluedark">{n.title}</h3>
                    <p className="text-[12.5px] font-semibold text-brand-gold">{n.date}</p>
                  </div>
                </div>
                <p className="mt-3 line-clamp-2 text-[13px] leading-relaxed text-muted">{n.description}</p>
          ...
          >
            <div className="flex items-center justify-between gap-3 border-b border-divider px-5 py-3">
              <div className="min-w-0">
                ...
                <p className="text-[12px] text-muted">{preview.date}</p>
              </div>
            ...
            <iframe src={preview.pdf} title={`${preview.title} — ${preview.date}`} className="h-full w-full flex-1 bg-pagebg" />
          </div>
        </div>
      )}
    </main>
