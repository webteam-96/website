// @ts-nocheck
/* Vendored from React Bits (reactbits.dev) — DavidHDev/react-bits: Components/CircularGallery. WebGL (ogl) gallery. */
...
class Media {
  ...
  update(scroll, direction) {
    ...
    // Curved-screen ("curved TV"): items stay level (no vertical rainbow arc).
    // They recede in depth toward the edges and turn around the vertical axis,
    // so the centre faces straight on and the sides wrap away from the viewer.
    if (this.bend === 0) {
      ...
      this.plane.position.y = 0;
      if (this.bend > 0) {
        this.plane.position.z = -arc;
        this.plane.rotation.y = -Math.sign(x) * Math.asin(effectiveX / R);
      } else {
        this.plane.position.z = arc;
        this.plane.rotation.y = Math.sign(x) * Math.asin(effectiveX / R);
      }
