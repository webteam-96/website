// @ts-nocheck
/* Vendored from React Bits (reactbits.dev) — DavidHDev/react-bits: Components/DomeGallery. 3D drag sphere (@use-gesture). */
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    // Continuous idle auto-spin around the vertical axis. Pauses while the user
    // is dragging, while a fling's inertia is still running, or while a photo is
    // enlarged — so the globe is always gently turning but stays grabbable.
    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    let raf = 0;
    const AUTO_SPEED = 0.08; // degrees per frame
    const spin = () => {
      raf = requestAnimationFrame(spin);
      if (draggingRef.current || inertiaRAF.current || focusedElRef.current) return;
      rotationRef.current = { x: rotationRef.current.x, y: rotationRef.current.y + AUTO_SPEED };
      applyTransform(rotationRef.current.x, rotationRef.current.y);
    };
    raf = requestAnimationFrame(spin);
